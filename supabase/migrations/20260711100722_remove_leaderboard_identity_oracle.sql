-- Block suppression is derived only from the authenticated viewer's social
-- graph. Callers cannot supply arbitrary Auth UUIDs as exclusion probes.

drop function if exists public.read_leaderboard(text,text,text,integer,text,uuid,uuid[],uuid[]);
