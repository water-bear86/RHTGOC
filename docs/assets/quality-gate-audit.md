...
| --- | --- | --- | --- |
| KayKit Merry Band | Accept for role conversion: Ranger, Rogue, Barbarian, and Hooded Rogue use the same exact 23-bone `Rig_Medium` rest skeleton as Character Animations 1.1, native weighted closed-hand meshes, one palette texture each, and explicit CC0 licenses. The project owner additionally supplied and positioned Robin's hat and feather. | Accept as the active Robin, Marian, Little John, and Much visuals after deterministic recolouring, embedded WebP export, four-clip packaging, and deformation review. Every hero carries an upright morphable bow on `handslot.l`; `Idle`, held-bow movement, the complete 0.6-second load followed by 0.4-second release, unchanged role signatures, the string-release seam, and both side views are verified without retargeting or body reshaping. Robin's hat and feather are directly attached to `head`, preserve the project owner's exact placement, and remain seated through all four exported clips. | `tools/build-kaykit-characters.py`, `docs/assets/licenses/kaykit-adventurers-and-character-animations-cc0.txt`, `docs/operations/3d-assets.md`, the four measured manifest entries, Blender deformation renders, and the browser family-photo audit. |
...
The project owner supplied Meshy rollback, treasure chest, and Robin hat/feather do not embed standard license identifiers. Their manifest status is therefore `project-authorized`, not a claim of CC0, an SPDX grant, or permission for unrelated reuse. Robin's underlying KayKit Ranger and animations remain CC0, but the combined shipping GLB takes the more restrictive project-authorized status because it includes the supplied headwear. KayKit Adventurers, KayKit Character Animations 1.1, and the Stylized Nature tree pack are otherwise different: their supplied licenses explicitly grant CC0 1.0, so converted artifacts containing only those sources record that standard identifier. A missing owner directive, incompatible third-party terms, or unknown source remains a rejection under the gate.
