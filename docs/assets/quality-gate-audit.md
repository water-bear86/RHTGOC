...
| --- | --- | --- | --- |
| Meshy Robin Hood | Rework: the desired Robin silhouette arrived as a 100,913-vertex static mesh with no armature or clips and oversized PBR textures. | Accept after a 22-bone skin bind, grounded pivot, Idle/Walk/Attack authoring, 35% topology simplification, removal of toon-irrelevant PBR maps, 1024 WebP base color, Meshopt compression, and runtime clip aliases. | `tools/rig-forest-ranger.py`, `tools/strip-robin-pbr.mjs`, and the manifest entry. The project owner supplied `Meshy_AI_Robin_Hood_0711214327_image-to-3d-texture (1).blend` and explicitly directed its in-game integration. |
...
The owner-supplied Meshy character does not embed a standard license identifier. Its manifest status is therefore `project-authorized`, not a claim of CC0, an SPDX grant, or permission for unrelated reuse. The Stylized Nature tree pack is different: its supplied license explicitly grants CC0 1.0, so the curated tree catalog records that standard identifier. A missing owner directive, incompatible third-party terms, or unknown source remains a rejection under the gate.
